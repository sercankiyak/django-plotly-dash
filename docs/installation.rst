...
.. note::
    An easy way to ensure the Plotly app works is to import it into ``views.py`` or ``urls.py`` as in the following example,
    which assumes the ``plotly_app`` module (``plotly_app.py``) is located in the same folder as ``views.py``.::

    from . import plotly_app
    
    Note: Your coding software might think that this import line is unused and redundant. However, it will actually let Django
    to connect and run your django-plotly-dash code. 
